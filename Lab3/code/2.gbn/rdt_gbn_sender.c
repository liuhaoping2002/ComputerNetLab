...
int prepare_send_window(FILE *fp) {
    ...
    if(ptr_pkt_left->state == RDT_PKT_ST_ACKED){
        ...
        while(max_acked < send_window.right && send_window.rdt_pkts[max_acked % send_window.len].state == RDT_PKT_ST_ACKED){
            max_acked++;
        }
    ...
    int new_pkt_seq;
    for( 	new_pkt_seq = send_window.right; 
				new_pkt_seq < (send_window.left + send_window.len); 
				new_pkt_seq++ )
		{
				...
				StatePkt *ptr_pkt_new = &send_window.rdt_pkts[new_pkt_seq % send_window.len];
