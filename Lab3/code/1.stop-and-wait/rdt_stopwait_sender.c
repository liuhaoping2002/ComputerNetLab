...
int deliver_file(char *send_file_name, int sockfd) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            int mypoll = poll(&pollfd, 1, 5);
            if(mypoll == -1){
                puts("Socket Error");
                return 1;
            }
            else if(mypoll == 0){
                puts("Time out, resend");
            }
            else{
                char buf[RDT_PKT_LEN];
                int reply_pkt = recv(sockfd, buf, RDT_PKT_LEN, 0);
                int reply_seq_num, reply_flag;
                unpack_rdt_pkt(NULL, buf, reply_pkt, &reply_seq_num, &reply_flag);
                if(reply_seq_num == seq_num && reply_flag == RDT_CTRL_ACK)
                    break;
            }
        }
